            ...
            <div className="flex items-center gap-3" ref={dropdownRef}>
                ...
                >
                    ...
                    <div className="w-9 h-9 rounded-xl overflow-hidden border border-slate-200 shadow-md group-hover:scale-110 transition-all duration-500">
                        <img
                            src={user?.photo || user?.avatar ? (user?.photo?.startsWith('data:') || user?.photo?.startsWith('http') ? user.photo : (user?.avatar?.startsWith('data:') || user?.avatar?.startsWith('http') ? user.avatar : `https://task-matrix-backend.vercel.app/img/users/${user?.photo || user?.avatar}`)) : `https://ui-avatars.com/api/?name=${user?.name || 'User'}&background=121212&color=fff`}
                            alt="Profile"
